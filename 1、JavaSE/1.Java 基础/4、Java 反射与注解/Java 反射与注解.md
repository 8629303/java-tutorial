    ...
    public static void main(String ...args) throws NoSuchFieldException, IllegalAccessException {
        {
             ...
             */
            // 获取Member的name字段
            Field field = Member.class.getDeclaredField("value");
            // 获取的Name注解实例
            Name name = field.getAnnotation(Name.class);
            // 获取name这个代理实例所持有的 InvocationHandler
            ...
            // 修改 value 属性值
            memberValues.put("value", "customFiledName");
            // 获取 name 的 value 属性值
        ...
        System.out.println("---------------------------分割线---------------------------");
        {
            ...
            Map memberValues = (Map) hField.get(h);
            // 修改 value 属性值
            memberValues.put("value", "customTypeName");
            // 获取 name 的 value 属性值
            System.out.println(name.value());
        }
        System.out.println("---------------------------分割线---------------------------");
...
> 如果是Java17的话需要增加如下JVM启动参数：
>
> ```bash
> --add-opens java.base/sun.reflect.annotation=ALL-UNNAMED --add-opens java.base/java.lang=ALL-UNNAMED
> ```
 ...
 * title: 如何提升反射的性能
 *
 * Java反射在许多知名框架中广泛应用，但在高CPU占用时性能较慢。
 * 通过缓存描述对象和跳过JVM内部安全检查，可以提高反射性能。反射的通用性和灵活性使其在框架中具有重要价值。
 * 总结：缓存描述对象和跳过安全检查可提升性能，反射具有通用性和灵活性
 */
public class ReflectionExample {
    ...
    /**
     * 方案一：普通创建对象方式，循环1000万次，耗时6毫秒
     */
    public static void createUserWithNew() {
        User user = new User();
    }

    /**
     * 方案二：反射创建对象方式，耗时407毫秒，原因时每次获取user实例都重新创建一个Constructor
     */
    public static void createUserWithReflect() {
        try {
    ...
    /**
     * 方案三：性能优化方案，缓存constructor，跳过JVM内部安全检查
     */
    private static Constructor<User> declaredConstructor;

    static {
        try {
            declaredConstructor = User.class.getDeclaredConstructor();
            // 设置访问不检查，注意：设置或者不设置耗时也有所不同
    ...
    public static void createUserWithReflectCache() {
        try {
            User user = declaredConstructor.newInstance();
