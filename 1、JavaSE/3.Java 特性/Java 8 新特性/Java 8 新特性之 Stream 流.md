...
操作示例 1：下面是一个简单的示例，演示如何使用 Stream 的 max() 方法来找到一组整数中的最大值：
...
public class StreamMaxExample {
    public static void main(String[] args) {
        ...
        // 使用 Stream 的 max() 方法找到最大值
        Optional<Integer> maxNumber = numbers.stream().max(Comparator.naturalOrder());
        // 输出结果
        maxNumber.ifPresent(max -> System.out.println("最大值是: " + max)); // 8
    }
}
```

操作示例 2：如果是一个对象数组的话，我们使用 Comparator.naturalOrder() 来指定自然顺序的比较器
...
// 输出内容
{李四=二班, 张三=一班, 王五=三班}
{李四=Student(name=李四, age=12, className=二班), 张三=Student(name=张三, age=10, className=一班), 王五=Student(name=王五, age=15, className=三班)}
```
