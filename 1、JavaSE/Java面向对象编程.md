...
**简介：** 本节将为读者介绍Java世界必不可少的特殊数据类型：String类。

**【本节目标】**通过阅读本节内容，你将初步了解String类的相关概念，并学会对String类型数据的简单定义与操作。
...
1. JDK 1.8：https://docs.oracle.com/javase/8/docs/api/overview-summary.html、https://docs.oracle.com/javase/8/docs/api/index.html
2. JDK 1.9：https://docs.oracle.com/javase/9/docs/api/overview-summary.html
3. JDK  10：https://docs.oracle.com/javase/10/docs/api/overview-summary.html
...
public class JavaAPIDemo {
    public static void main(String[] args) {
        ...
        System.out.println(msg.echo("www.xxxx.cn"));
    }
}
...
操作示例 3：观察throw的使用
与**throws**对应的还有**throw**关键字，此关键字主要作用表示手工进行异常的抛出，即：此时将手工产生一个异常类的实例化对象，并且进行异常的抛出处理。
...
从JDK1.8开始为了简化使用者进行代码的开发，专门提供有Lambda表达式的支持，利用此操作形式可以实现函数式的编程。对于函数式编程比较著名的语言：haskell、Scala，利用函数式编程可以避免掉面向对象编程之中的一些繁琐的处理问题。面向对象在其长期发展的过程之中一直有一部分的反对者，这些反对者认为面向对象的设计过于复杂，过于繁琐了。以一个最简单的程序为例。
...
操作示例 2：使用 Lambda表达式实现与之前完全一样的功能
...
Lambda表达式如果要想使用，那么必须有一个重要的实现要求：SAM（Single abstract method），只有一个抽象方法，以之前的 IMessage 接口为例，在这个接口里面发现只是提供有一个send方法，除此之外没有任何的其它方法定义，所以这样的接口就被称为函数式接口，而只有函数式接口才可以被 Lambda表达式所使用。
...
利用Lambda表达式的确摆脱传统面向对象之中关于结构的限制，使得代码更加的简便。
...
操作示例 1：引用静态方法，在String类里面提供由String.valueOf() 方法，这个方法就属于静态方法。方法定义：public static String valueOf(int i)，该方法有参数，并且有返回值。利用方法引用这一概念可以为一个方法定义多个名字，但是要求必须是函数式接口。
...
操作示例 2：引用实例化对象中的方法，在String类里面有一个转大写的方法：public String toUpperCase()，这个方法是必须在有对象实例化提供的情况下才可以调用
...
public class JavaAPIDemo {
    public static void main(String[] args) {
        IFunction<String> fun = "www.xxxx.cn" :: toUpperCase;
        System.out.println(fun.upper());
    }
}
```

在进行方法应用的时候也可以引用特定类中的一些操作方法，在String类里面提供有一个字符串大小关系比较：`public int compareTo(String anotherString);`这是一个普通方法，如果要引用普通，则往往都需要实例化对象，但是如果说现在你不想给出实例化对象，只是想引用这个方法，则就可以使用特定类来进行引用处理。
...
在系统之中专门提供有一个：java.util.function 的开发包，里面可以直接使用函数式接口，在这个包下面一共有如下的几个核心接口供使用：
...
1、功能性函数式接口：有⼊参，有返回值。例如：在String类中有一个方法判断是否以指定的字符串开头：public boolean startsWith(String str)
...
2、消费型接⼝：有⼊参，⽆返回值。例如：在进行系统数据输出的时候使用的是：System.out.println();
...
public class JavaAPIDemo {
    public static void main(String[] args) {
        ...
        con.accept("www.xxxx.cn") ;	
    }
}
```

3、供给型接⼝：⽆⼊参，有返回值。例如：String类中的方法：public String toLowerCase();

```java
import java.util.function.*;
public class JavaAPIDemo {
    public static void main(String[] args) {
        Supplier<String> sup = "www.XXXX.cn" :: toLowerCase;
        System.out.println(sup.get());
    }
}
```

4、断⾔型接⼝：有⼊参，有返回值，返回值类型确定是boolean。例如：String类中的方法：public boolean equalsIgnoreCase();
